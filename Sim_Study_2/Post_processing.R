...
for(j in 1:4){
  ...
  for(i in 1:10){
    ...
    Y <- readRDS(paste("/Users/nicholasmarco/Box Sync/BayesFPMM_Supporting_Files/BMPMM_simulation/Optimal_K/data/data", i, ".RDS", sep = ""))
    Y <- Y$y
    AIC[i,j] <- MV_Model_AIC(dir_i, n_files, 1000, Y)
    BIC[i,j] <- MV_Model_BIC(dir_i, n_files, 1000, Y)
    DIC[i,j] <- MV_Model_DIC(dir_i, n_files, 1000, Y)
    x <- MV_Model_LLik(dir_i, n_files, 1000, Y)
